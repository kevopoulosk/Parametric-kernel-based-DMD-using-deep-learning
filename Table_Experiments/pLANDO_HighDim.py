...
class ParametricLANDO:
    ...
    def train_fnn(self, reduced_output, fnn_depth, fnn_width, reduced_state_mat, epochs, batch_size_frac, verbose):
        ...
        if self.problem == "heat":
            X_train = self.scaler_x.fit_transform(self.samples[self.train_samples].reshape(-1, 1))
            X_valid = self.scaler_x.transform(self.samples[self.valid_samples].reshape(-1, 1))
            X_test = self.scaler_x.transform(self.samples[self.test_samples].reshape(-1, 1))
        else:
            X_train = self.scaler_x.fit_transform(self.samples[self.train_samples, :])
            X_valid = self.scaler_x.transform(self.samples[self.valid_samples, :])
            X_test = self.scaler_x.transform(self.samples[self.test_samples, :])
        ...
        train_loader = DataLoader(dataset=dataset_train, batch_size=int(TrainSamples * batch_size_frac), shuffle=True)
        ...
        for epoch in range(epochs):
            ...
            ### Reduce the learning rate when we have validation error < 0.5%
            if mean_relative_err_val < 0.005:
        ...
        relative_errors = self.relative_error(y_test=y_test, prediction=prediction, tensor=True, mean=False)

        print(f"FNN Training, Mean test error, {TestSamples} samples: {np.mean(relative_errors)}")
        print(f"Execution time for DNN train {end_train - start_train}")

        return Mapping_FNN, X_train, y_train, X_test, y_test, relative_errors
